...
[SHALL:process](https://hl7.org/fhir/extensions/CodeSystem-obligation.html#obligation-SHALL.58process) | Indicates that when the element is included in a resource by the SHC App, it should be processed and considered for accurate writeback. | Indicates the SHC App will process the element and use it for prepopulating a QuestionnaireResponse.
